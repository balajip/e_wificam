...
#ifdef ONVIF_ENABLE
               /* Below API will initialize the onvif parameters and the discovery protocols */
               if(onvifInit() != 0)
               {
                       log_error_write(srv,__FILE__,__LINE__,"s","Error : Onvif functionality failed to initialize");
               }
#endif
