...
extern  bool IsServerMediaSessionGetChange(ServerMediaSession *);
int Stop[4] = {0};
...
::handleCmd_DESCRIBE(char const* urlPreSuffix, char const* urlSuffix, char const* fullRequestStr) {
  ...
  //added for support to stop reconnection after Teardown of clients
  if(ServerStop == true)     
  {      
      return;
  }
  ...
  do {
    ...
    {
        if (!IsServerMediaSessionGetChange(session) )
        {
            printf("Server MediaSession Get Change Done.\n");
            return ;
        }
    ...
    *fLastCRLF = '\r';
	...
	if (sessionIdStr[0] == '\0') {
	  ...
	  do {
	    ...
	    sprintf(sessionIdStr, "%08u", sessionId);
    ...
    if (clientSession != NULL && clientSession->fStreamAfterSETUP && strcmp(cmdName, "SETUP") == 0) {
      ...
      //clientSession->fStreamStates.subsession->SyncWithServerTimeStamp(fStreamStates.streamToken);
    }
...
RTSPServer::RTSPClientSession::~RTSPClientSession() {
  ...
  char sessionIdStr[200];
  sprintf(sessionIdStr, "%08u", fOurSessionId);
  ...
  if (fOurServerMediaSession != NULL) {
	...
	&& fOurServerMediaSession->deleteWhenUnreferenced()) {
	    //Stop[fOurServerMediaSession->SessionTrackID()] = 1;
	    //usleep(1000000);
...
::handleCmd_TEARDOWN(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  unsigned i;
  // Add for Teardowning the Sessions 
  if (fOurServerMediaSession->referenceCount() == 1)
      Stop[fOurServerMediaSession->SessionTrackID()] = 1;
      usleep(1000000);
...
::handleCmd_PLAY(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  Stop[fOurServerMediaSession->SessionTrackID()] = 0; //added for setting the Flag for Stopping the Server Media session
  ...
  //TO Syncronise With Server Tiestamps
  //fStreamStates[i].subsession->SyncWithServerTimeStamp(fStreamStates[i].streamToken);
...
# if 1
  ...
  //fprintf(stderr, "RTSP client session (id \"%08u\", stream name \"%s\"): Liveness indication\n",
  //	  clientSession->fOurSessionId, streamName);
#endif
printf("Liveness indication from:<%s>\n",streamName);
