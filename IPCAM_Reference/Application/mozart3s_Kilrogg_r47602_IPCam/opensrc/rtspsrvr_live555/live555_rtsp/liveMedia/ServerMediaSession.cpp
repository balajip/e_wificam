...
::createNew(UsageEnvironment& env,
	    ...
	    char const* description, Boolean isSSM, char const* miscSDPLines,int iTrackid) {
#ifdef FS_DEBUG
  printf("FS:file::%s,function::%s,line.no::%d\n",__FILE__,__FUNCTION__,__LINE__);
#endif
  return new ServerMediaSession(env, streamName, info, description,
				isSSM, miscSDPLines,iTrackid);
}
...
ServerMediaSession::ServerMediaSession(UsageEnvironment& env,
				       ...
				       Boolean isSSM, char const* miscSDPLines,int iTrackid)
  : Medium(env), fIsSSM(isSSM), fSubsessionsHead(NULL),
    fSubsessionsTail(NULL), fSubsessionCounter(0),
    fReferenceCount(0), fDeleteWhenUnreferenced(False),iTrackId(iTrackid) {
