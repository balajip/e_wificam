...
::MPEG4ESVideoRTPSink(UsageEnvironment& env, Groupsock* RTPgs, unsigned char rtpPayloadFormat, u_int32_t rtpTimestampFrequency,
		      u_int8_t profileAndLevelIndication,int configsize ,char const* configStr)
  ...
  //printf("NumofConfig:%x---->%d\n",configStr[3],(strlen(configStr)));
  //fConfigBytes = parseGeneralConfigStr(configStr, fNumConfigBytes);
   fNumConfigBytes = configsize;
  fConfigBytes = new unsigned char[configsize];
  memmove(fConfigBytes,configStr,fNumConfigBytes);
}
...
MPEG4ESVideoRTPSink::createNew(UsageEnvironment& env,
			       ...
			       u_int8_t profileAndLevelIndication, int Configsize,char const* configStr) {
  return new MPEG4ESVideoRTPSink(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency, profileAndLevelIndication,Configsize ,configStr);
}
