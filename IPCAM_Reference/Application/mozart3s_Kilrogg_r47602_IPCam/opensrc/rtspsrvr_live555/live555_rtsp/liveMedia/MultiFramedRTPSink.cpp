...
void MultiFramedRTPSink::sendPacketIfNecessary() {
  ...
  if (fNoFramesLeft) {
...
#if 0
    ECodecType eCodecType = fSource->getCodecType();
    switch( eCodecType )
    {
        case  CODEC_TYPE_MJPG :
        { 
            if( uSecondsToGo > 200000) 
            {
                printf("WARNING delay>200000 to schedule the next task  MJPG \n");
                uSecondsToGo = 40000;
            }
...
#endif
    iTrackId = fSource->GetTrackIdFromSource();
#if 1
    if (iTrackId == 0)
    {
            if(Stop[iTrackId] == 0 )
                nextTask() = env1->taskScheduler().scheduleDelayedTask(uSecondsToGo, (TaskFunc*)sendNext, this);
            else
              goto Default;
    }
    else if(iTrackId == 1)
    {
            if(Stop[iTrackId] == 0)
            nextTask() = env2->taskScheduler().scheduleDelayedTask(uSecondsToGo, (TaskFunc*)sendNext, this);
            else
              goto Default;
    }
    else if(iTrackId == 2)
    {
            if(Stop[iTrackId] == 0)
            nextTask() = env3->taskScheduler().scheduleDelayedTask(uSecondsToGo, (TaskFunc*)sendNext, this);
            else
              goto Default;
    }
    else if(iTrackId == 3)
    {
            if(Stop[iTrackId] == 0)
            nextTask() = env4->taskScheduler().scheduleDelayedTask(uSecondsToGo, (TaskFunc*)sendNext, this);
            else
              goto Default;
    }
    else
