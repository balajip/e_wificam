/*
 * $Header: /rd_2/project/SoC/Linux_Libraries/VideoCap/VideoCap_App/VideoCap_App.c 93    10/09/09 6:52p Ycchang $
 *
...
int main(int argc, char* argv[])
{
	...
	DWORD dwDummy;
	DWORD dwCurFrameTime=0;
	DWORD dwPreFrameTime=0;
	...
	if (argc != 4) {
		...
		printf("operation_mode: 1 -> capture-only(master only)\n");
		printf("                2 -> retrieve-only(slave only)\n");
...
//The following code is for reproducing Bug 239: [SDK4.0][EVM_M380A_Ver.C][OV9715]Initialization default, then frame rate is not enough in low lux
sleep(10);
...
if (bServer == TRUE) {
      ...
      tVideoSignalOptions.adwUserData[0] = 30;
	printf("[videocap_tester](%d)jeff MaxShutter=%d\n",__LINE__,30);
	if (VideoCap_SetOptions(hVideoCapObject, &tVideoSignalOptions) != S_OK) {
		  printf("videocap_tester] set VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MAX_SHUTTER fail!!\n");
	} else {
		  printf("[videocap_tester] set VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MAX_SHUTTER success!!\n");
	}
	
	
	    memset(&tVideoSignalOptions,0x0,sizeof(TVideoSignalOptions));
      tVideoSignalOptions.eOptionFlags = VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MIN_SHUTTER;
      tVideoSignalOptions.adwUserData[0] = 120;
	printf("[videocap_tester](%d)jeff MinShutter=%d\n",__LINE__,120);
	if (VideoCap_SetOptions(hVideoCapObject, &tVideoSignalOptions) != S_OK) {
		  printf("videocap_tester] set VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MIN_SHUTTER fail!!\n");
	} else {
		  printf("[videocap_tester] set VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MIN_SHUTTER success!!\n");
	}
	
	
         memset(&tVideoSignalOptions,0x0,sizeof(TVideoSignalOptions));
      tVideoSignalOptions.eOptionFlags = VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MAX_GAIN;
      tVideoSignalOptions.adwUserData[0] = 32;
	printf("[videocap_tester](%d)jeff MaxGain=%d\n",__LINE__,32);
	if (VideoCap_SetOptions(hVideoCapObject, &tVideoSignalOptions) != S_OK) {
		  printf("videocap_tester] set VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MAX_GAIN fail!!\n");
	} else {
		  printf("[videocap_tester] set VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MAX_GAIN success!!\n");
	}
      
      memset(&tVideoSignalOptions,0x0,sizeof(TVideoSignalOptions));
      tVideoSignalOptions.eOptionFlags = VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MIN_GAIN;
      tVideoSignalOptions.adwUserData[0] = 2;
	printf("[videocap_tester](%d)jeff MinGain=%d\n",__LINE__,2);
	if (VideoCap_SetOptions(hVideoCapObject, &tVideoSignalOptions) != S_OK) {
		  printf("videocap_tester] set VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MIN_GAIN fail!!\n");
	} else {
		  printf("[videocap_tester] set VIDEO_SIGNAL_OPTION_SET_AUTO_EXPOSURE_MIN_GAIN success!!\n");
	}
	...
	for (i=0; i<dwFrameCount; i++) {
		...
		dwCurFrameTime=tVideoCapState.dwSecond*1000*1000+tVideoCapState.dwMilliSecond*1000+tVideoCapState.dwMicroSecond;
		printf("Time diff =%u between frame %d and frame %d\n",(dwCurFrameTime-dwPreFrameTime),dwPrevFrameCount,tVideoCapState.dwFrameCount);
