/*
 * $Header: /rd_2/project/SoC/Components/VIC/Device_Driver/VIC/global_codec.h 144   10/06/01 6:05p Evelyn $
 *
...
struct video_sensor_initial_param {
	/* Sensor light frequency (50 Hz or 60 Hz) */
	int video_sensor_frequency;
	/* Sensor capture width */
	int video_sensor_width;
	/* Sensor capture height */
	int video_sensor_height;
};

/* AE table information */
struct auto_exposure_table_info {
	//shutter related
	unsigned int dwShutterTableNum;
	unsigned int dwFlickerlessShutterIdx;
	unsigned int dwFrameRateShutterIdx;
	// gain comp related
	unsigned int dwGainCompTableNum;
	unsigned long* apdwGainCompTable[3];
};

/* sensor call back function */
struct video_sensor_device {
	int (*open)(struct video_sensor_initial_param* param, unsigned long devNum);
	void (*release)(unsigned long devNum);
	int (*ioctl)( TVideoSignalOptions* arg, unsigned long devNum);
	void (*get_ae_tbl_info)(struct auto_exposure_table_info* info);
	unsigned int (*get_shutter_value)(void);
};

/* AE initial parameters */
struct auto_exposure_initial_param {
	/* AE Keep frame rate enable (SWAEMode) */
	int bKeepFrameRateEn;
	/* AE Keep flickerless enable (SWAEMode) */
	int bKeepFlickerlessEn;
	/* AE Numbers of AE operating every N frames (SWAEMode) */
	int dwAEopframes;	
	/* AE Target luminace (SWAEMode) */
	int dwTargetLuminance;
	/* AE Luminace offset (SWAEMode) */
	int dwTargetOffset; // should be greater than FINE_TUNE_RANGE (5)
	/* AE Luminance threshold for environment big change (SWAEMode) */
	int dwEBCThreshold;
	/* AE Increased step for environment big change (SWAEMode: related to EBCGainIdxStep) */
	int dwEBCGainIdxStep;
	/* AE Gain step number for shutter compensation (SWAEMode) */
	int dwAEGainIdxStepShCompen;
	/* AE Shutter default width */
	int dwDefaultShutter;
	...
	int dwMaxGain;
	/* AE Shutter gain compensation delay frame num */
	int dwShutterGainCompenDelayFrameNum;
	/* AE shutter/gain table info */
	struct auto_exposure_table_info* ae_table;
};

/* AE status structure */
typedef struct auto_exposure_state
{
	EVideoSignalOptionFlags eAEOptionFlags;
	unsigned long dwTargetLuminance, dwTargetOffset;
	unsigned long dwFrameRate, dwGain;
	unsigned long *pdwShutterTable;
	unsigned long dwHWAELuminance;
	unsigned long dwWindowPixels;
	signed long sdwCurrShutterIdx, sdwCurrGainIdx;
	unsigned long bShutterValid, bGainValid;
	DWORD devNum;	// AE, for tasklet
} TAutoExposureState;

/* AE call back function */
struct auto_exposure_module {
	int (*open)(struct auto_exposure_initial_param* param);
	int (*release)(void);
	int (*set_options)(TVideoSignalOptions *arg);
	void (*one_frame)(TAutoExposureState *state);
};
/* ====================================================================================== */
#endif // codec
