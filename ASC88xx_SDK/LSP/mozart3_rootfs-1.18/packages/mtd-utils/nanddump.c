...
int main(int argc, char * const argv[])
{
	...
	for (ofs = start_addr; ofs < end_addr ; ofs+=bs) {
		...
		if (badblock) {
			if (omitbad) {
				fprintf(stderr, "Skip the bad block 0x%08x, and move to next block..\n", ofs);    
				if((end_addr + bs) <= meminfo.size) {
					fprintf(stderr, "- We externd end addr from 0x%08x to 0x%08x\n", end_addr, end_addr + meminfo.erasesize) ;
					//ofs += meminfo.erasesize ;
					//end_addr += meminfo.erasesize ;
					end_addr += bs ;
				} else
					fprintf(stderr, "- We use the original end addr") ;
