...
int dram_init(void)
{
	...
	for (i=0, ddrctrl=DDR2SDMC0_MMR_BASE; i < CONFIG_DRAM_NUMBER; i++, ddrctrl += 0x01000000)
	{
		if (readl(ddrctrl + DDR_CTRL) & EN(1))
		{
			int v = readl(ddrctrl + DDR_SIZE);
			gd->bd->bi_dram[v >> 28].start = v & BASE_ADDR(0xff000000);
			gd->bd->bi_dram[v >> 28].size = CALC_DDR_SIZE(v);
		}
	}
#else
    ...
    config_reg_val = v_inl(EVM_DDR2SDMC0_BASE + EVM_SDRAMC_SIZE_CFG);
