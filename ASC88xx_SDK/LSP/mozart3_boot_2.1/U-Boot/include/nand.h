...
struct nand_erase_options {
	...
	int scrub;		/* if true, really clean NAND by erasing
				 * bad blocks (UNSAFE) */
    //[jam patch]
    //By default, we erase the nand flash in the limited region.(To protect the ubifs region)
    //But sometimes we need to erase all blocks and skip bad blocks.
    //If all == 1 : erase whole nand chip and skip bad.
    //If all == 0 : erase in the limited region
    //scrub is to erase all blocks including bad blocks.(it's dangerous)
    int all;  
};
