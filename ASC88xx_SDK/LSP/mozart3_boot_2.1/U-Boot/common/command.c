...
int
do_reserved (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	extern unsigned long malloc_start ;
	extern unsigned long malloc_end ;
	extern unsigned long stack_start ;
	extern unsigned long stack_end ;
	extern volatile unsigned long DMA_BUFFER_BASE ;
	extern unsigned long gmac_dma_buf_end ;

	printf("\n");
	printf("  ===== U-Boot DDR Reserved Area =====\n");
	...
	printf("\n") ;
	printf("  !! Please do not write any data to these space. !!\n\n") ;	

	return 0;
}

U_BOOT_CMD(
	reserved,	1,		1,	do_reserved,
	"print reserved area",
	NULL
);
