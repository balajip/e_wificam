...
void nand_select_chip(struct mtd_info *mtd, int chipnumber)
{
	...
	setting |= (chipnumber << 16) | NF_FLCTRL_ECC_ENABLE ;
...
#define VPL_NFC_FSM_BUSY_MASK	0x60
#define VPL_NFC_RnB_BUSY_MASK	( VPL_NFC_FSM_BUSY_MASK | VPL_NFC_RnB_READY )
...
static void nand_wait_reset( struct mtd_info *mtd)
{
	...
	while (timeout >= 0){
		if(chip->read_byte(mtd) & NAND_STATUS_READY) {
			//printf( "In Beethoven_nand_wait_reset(), It's ready!!") ;
			return ;
		}
		timeout-- ;
	}
...
static void nand_dma_ready( struct mtd_info *mtd )
{
	...
	do {
		...
		status &= VPL_NFC_DMA_BUSY_MASK ;//check DMA transfer done
		status &= VPL_NFC_FSM_BUSY_MASK ;//check FS IDLE
	} while(status != 0) ;
}
...
#define VPL_NFC_DMA_BURSTTYPE    ( 2 << 11 ) // single burst address incement
#define VPL_NFC_DMA_TRANSFERSIZE ( 0 << 14 ) // 8-bits transfer size
...
#if 0
int Beethoven_nand_dma_transfer(struct mtd_info *mtd, const uint8_t *buf, unsigned long sram_addr, int len, unsigned long dir)
{
...
int nand_dma_transfer_SRAM2AHB(struct mtd_info *mtd, uint8_t *buf, unsigned long sram_addr, int len)
{
	// In our NFC, we have read a page to internal buffer by Page-read command.
	// So the task of this function is "using dma"
	// to transfer data from "internal buffer" to "external memory".
...
int nand_dma_transfer_AHB2SRAM(struct mtd_info *mtd, const uint8_t *buf, unsigned long sram_addr, int len)
{
	// In our NFC, we have read a page to internal buffer by Page-read command.
	// So the task of this function is "using dma"
	// to transfer data from "internal buffer" to "external memory".
...
static int nand_ecc_read_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf)
{
	...
	if ( !(chip_priv->dma_mode) ) {
		...
		//Read Data Part
		nand_dma_transfer_SRAM2AHB(mtd, buf, 0, mtd->writesize);
		//Read OOB Part
		nand_dma_transfer_SRAM2AHB(mtd, chip->oob_poi, mtd->writesize, mtd->oobsize);
	}
...
static void nand_ecc_write_page_hwecc(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t *buf)
{
	...
	nand_dma_transfer_AHB2SRAM(mtd, buf, 0, mtd->writesize);
	}
...
static int vpl_nand_scan(struct nand_chip *chip, struct vpl_nand_chip_private *chip_priv)
{
	...
	//chip->ecc.layout = &Beethoven_nand_oob_64 ;
...
int board_nand_init(struct nand_chip *chip)
{
	...
	if (chip_priv == NULL) {
		printf( "[ERR]No memory for chip_priv of Beethoven_nand\n" );
		err = -ENOMEM;
