...
int nand_util_bad_markbad(nand_info_t *nand, unsigned long addr) 
{
...
static size_t get_len_incl_bad (nand_info_t *nand, size_t offset,
				const size_t length)
{
	size_t len_incl_bad = 0;//[jam trace] how many bytes of device will be occupied(includes bad block) ?
	size_t len_excl_bad = 0;//[jam trace] how many bytes of file can be written successfully ?
	size_t block_len;
			...
			u_char *buffer)
{
	...
	if ((offset & (nand->writesize - 1)) != 0 ||
		...
		if((offset & (nand->writesize - 1)) != 0)
			printf("offset = %d, nand->writesize  = %d\n", (int)offset, nand->writesize) ;
		if((*length & (nand->writesize - 1)) != 0)
			printf("*length = %d, nand->writesize  = %d\n", *length, nand->writesize) ;
		return -EINVAL;
	}
	...
	while (left_to_write > 0) {
        ...
        //Avoid writing data to bad block
		if (nand_block_isbad (nand, offset & ~(nand->erasesize - 1))) {
