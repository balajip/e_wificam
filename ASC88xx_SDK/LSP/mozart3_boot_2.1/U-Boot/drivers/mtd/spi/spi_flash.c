...
{
#ifdef CONFIG_DW_SPI
	unsigned long flags = SPI_XFER_BEGIN | DW_SPI_XFER_EEPROM_READ ;// conflict with spansion 32M, wait analysis | DW_SPI_DSF_16;
#else
	unsigned long flags = SPI_XFER_BEGIN;
#endif
	int ret;
...
{
#ifdef CONFIG_DW_SPI
	unsigned long flags = SPI_XFER_BEGIN | DW_SPI_XFER_TX_ONLY;
#else
	unsigned long flags = SPI_XFER_BEGIN;
#endif
	int ret;
