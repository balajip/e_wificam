...
int genphy_update_link(struct phy_device *phydev)
{
	...
	if ((phydev->autoneg == AUTONEG_ENABLE) &&
	    (mii_reg & BMSR_ANEGCAPABLE) &&
	    !(mii_reg & BMSR_ANEGCOMPLETE)) {
		int i = 0;

		DEBUGF("%s Waiting for PHY auto negotiation to complete",
			phydev->dev->name);
...
#if 0 /* [Vincent] not define miiphy_get_dev_by_name, but also does't need this function. Whatever, comment out */
