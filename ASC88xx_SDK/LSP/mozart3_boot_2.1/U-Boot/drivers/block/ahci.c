...
static inline unsigned long ahci_port_base(unsigned long base, unsigned long port)
{
...
static int ahci_host_init(struct ahci_probe_ent *probe_ent)
{
	...
	for (i = 0; i < probe_ent->n_ports; i++) {
		probe_ent->port[i].port_mmio = ahci_port_base((unsigned long) mmio, i);
...
static void ahci_print_info(struct ahci_probe_ent *probe_ent)
{
	pci_dev_t pdev = probe_ent->dev;
	volatile u8 *mmio = (volatile u8 *)probe_ent->mmio_base;
	unsigned long vers, cap, impl, speed;
	const char *speed_s;
	u16 cc;
	const char *scc_s;
...
static int ahci_init_one(pci_dev_t pdev)
{
	...
	probe_ent->mmio_base = (unsigned long)pci_map_bar(pdev, AHCI_PCI_BAR,
						PCI_REGION_MEM);
...
static void ahci_fill_cmd_slot(struct ahci_ioports *pp, unsigned long opts)
{
...
static void ahci_set_feature(u8 port)
{
	...
	unsigned long cmd_fis_len = 5;	/* five dwords */
	u8 fis[20];
...
static int ahci_port_start(u8 port)
{
	...
	writel_with_flush((unsigned long) pp->cmd_slot, port_mmio + PORT_LST_ADDR);
...
static int ata_scsiop_read10(ccb * pccb)
{
	...
	len = (((unsigned long) pccb->cmd[7]) << 8) | ((unsigned long) pccb->cmd[8]);
...
static int ata_scsiop_read_capacity10(ccb *pccb)
{
	...
	*(unsigned long *) buf = le32_to_cpu(ataid[pccb->target]->lba_capacity);
