...
#ifndef    __MOZART_UGPIO_H
#define    __MOZART_UGPIO_H

#include <mach/platform.h>

extern spinlock_t mozart_ugpio_lock;
extern u32 mozart_ugpio_requested;
...
{
    if ((mozart_ugpio_requested & ugpio_pin_to_bit(pin)) != 0) {
        return -EBUSY;
    } else {
        spin_lock(&mozart_ugpio_lock);
        mozart_ugpio_requested |= ugpio_pin_to_bit(pin);
        spin_unlock(&mozart_ugpio_lock);
    }
