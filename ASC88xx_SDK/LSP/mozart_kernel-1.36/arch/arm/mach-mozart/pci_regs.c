...
#ifdef CONFIG_PCIEDEV_AR9280    
static void AR9280_regmap_init(unsigned int *dev_regs)
{
    /* set up PCI compatible configuration space mask (under 0x40) */
    memcpy(dev_regs, pci_type0_cfghdr, 4*16); /* type 0 configuration space header */
    dev_regs[4] = 0xffff0000; /* BAR0 */
    ...
    memcpy(&(dev_regs[0x10]), pci_pmcap_regs, 4*2);    /* PM capability on 0x40 */
    memcpy(&(dev_regs[0x14]), pci_msicap_regs, 4*4);   /* MSI capability on 0x50 */
    memcpy(&(dev_regs[0x18]), pci_pciecap_regs, 4*13); /* PCI Express capability on 0x60 */
}
#endif
...
#ifdef CONFIG_PCIEDEV_AR9280   
    {
        .bus = 1,       // Atheros 802.11n WiFi card(2T2R)
        .devfn = PCI_DEVFN(0, 0),
        .vendor_id = 0x168C,
        .device_id = 0x002a,
        .dev_regs = NULL,
        .regmap_init = AR9280_regmap_init,
    },
#endif
	{
		.vendor_id = 0xffff,
...
void mozart_pci_regmap_init(pdev_ids *pdev)
{
...
unsigned int mozart_pci_getmask(unsigned char bus, unsigned int devfn, int where)
{
	pdev_ids *pdev_id;
	int found;
	...
	if (found == 0) {
		printk("[PCIE] can not find register mask!!\n");
		while(1);
	}
