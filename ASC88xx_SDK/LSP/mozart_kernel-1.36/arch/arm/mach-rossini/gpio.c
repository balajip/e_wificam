...
wait_queue_head_t gpio_wq[ROSSINI_N_GPIO];
static volatile unsigned long g_old_jiffies[ROSSINI_N_GPIO];
static unsigned long g_GPIO_debounce_time = 1;

static DECLARE_BITMAP(gpio_in_use, ROSSINI_N_GPIO);
...
static irqreturn_t GPIO_isr(int irq, void *dev_id)
{
	...
	for (queue_num=0; queue_num<ROSSINI_N_GPIO; queue_num++)
	{
...
static void rossini_gpio_exit(void)
{
	dev_t dev;
...
/*-------------------------------------------------------*/
static int  rossini_gpio_init(void)
{
	...
	{
		dev = MKDEV(gsdwMajor, 0);
		ret = register_chrdev_region(dev, ROSSINI_N_GPIO, "gpio");
		if (ret < 0)
		{
			goto fail_init;
		}
	...
	{
		ret = alloc_chrdev_region(&dev, 0, ROSSINI_N_GPIO, "gpio");
		if (ret < 0)
		{
			goto fail_init;
		}
	...
	ret = cdev_add(gpio_cdev, dev, ROSSINI_N_GPIO);
	if (ret < 0) {
		goto fail_init;
	}
...
module_init(rossini_gpio_init);
module_exit(rossini_gpio_exit);
