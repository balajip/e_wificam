...
static DECLARE_BITMAP(agpo_in_use, ROSSINI_N_AGPO);
...
/*-------------------------------------------------------*/
static void rossini_agpo_exit(void)
{
	...
	if (gsdwMajor!=0)
	{
        ...
        unregister_chrdev_region(dev, ROSSINI_N_AGPO);
	}
...
/*-------------------------------------------------------*/
static int  rossini_agpo_init(void)
{
	...
	{
		dev = MKDEV(gsdwMajor, 0);
		ret = register_chrdev_region(dev, ROSSINI_N_AGPO, "agpo");
		if (ret < 0)
	    {
    	    goto fail_init;
		}
	...
	{
		ret = alloc_chrdev_region(&dev, 0, ROSSINI_N_AGPO, "agpo");
		if (ret < 0)
	    {
    	    goto fail_init;
		}
    ...
    ret = cdev_add(agpo_cdev, dev, ROSSINI_N_AGPO);
...
/*-------------------------------------------------------*/
module_init(rossini_agpo_init);
module_exit(rossini_agpo_exit);
