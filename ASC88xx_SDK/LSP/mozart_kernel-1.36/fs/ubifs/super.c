...
static int mount_ubifs(struct ubifs_info *c)
{
	...
	c->dbg_buf = Mozart_vmalloc(c->leb_size);
	if (!c->dbg_buf)
		return -ENOMEM;
#endif
	...
	c->sbuf = Mozart_vmalloc(c->leb_size);
	if (!c->sbuf)
		goto out_free;

	if (!mounted_read_only) {
		c->ileb_buf = Mozart_vmalloc(c->leb_size);
		if (!c->ileb_buf)
			goto out_free;
	}
...
out_free:
	kfree(c->bu.buf);
	Mozart_vfree(c->ileb_buf);
	Mozart_vfree(c->sbuf);
	kfree(c->bottom_up_buf);
	UBIFS_DBG(Mozart_vfree(c->dbg_buf));
	return err;
}
...
static int ubifs_remount_rw(struct ubifs_info *c)
{
	...
	c->orph_buf = Mozart_vmalloc(c->leb_size);
	if (!c->orph_buf) {
		err = -ENOMEM;
		goto out;
	}
