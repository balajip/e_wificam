...
{
        data[0] = 2;
        data[1] = UVC_STREAM_EOH | UVC_STREAM_FID;
...
//	printk("\r\n uvc_video_pump");
		...
		//printk("inside uvc_video_pump\r\n");
                ...
                video->encode(req, video, buf);
//		printk("Queue the USB request \r\n");
                /* Queue the USB request */
                if ((ret = usb_ep_queue(video->ep, req, GFP_KERNEL)) < 0) {
                        printk(KERN_INFO "Failed to queue request (%d)\n", ret);
                        printk("Failed to queue request (%d)\n", ret);
