...
#ifdef CONFIG_USB_EHSET
static void EHSET_tests(struct work_struct *data)
{
	struct usb_device	*udev = container_of(data, struct usb_device ,EHSET_work);
		...
		if (EHSET_workqueue) {
			if (down_trylock(&ehset_sem) == 0) {
				if (!hcd->EHSET_in_progress) {
					...
					up(&ehset_sem);
					INIT_WORK(&udev->EHSET_work, EHSET_tests);
