...
static void stmmac_clk_csr_set(struct stmmac_priv *priv)
{
	...
	clk_rate = AHB_CLK;/*clk_get_rate(priv->stmmac_clk);*/
...
static int stmmac_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (unlikely(stmmac_tx_avail(priv) < ((nfrags>>1) + 1))) {
	...
	for (i = 0; i < nfrags; i++) {
		...
		if (i&1) {			
			desc->des2 = dma_map_page(priv->device, frag->page,
						  frag->page_offset,
						  len, DMA_TO_DEVICE);
			desc->des01.tx.buffer1_size = len;
		} else {
			desc->des3 = dma_map_page(priv->device, frag->page,
						  frag->page_offset,
						  len, DMA_TO_DEVICE);
			desc->des01.tx.buffer2_size = len;
		}

		//priv->hw->desc->prepare_tx_desc(desc, 0, len, csum_insertion);
		if (likely(csum_insertion))
			desc->des01.tx.checksum_insertion = cic_full;

		if (i&1) { //skip first segment in first desc
			priv->tx_skbuff[entry] = NULL;
			desc->des01.tx.first_segment = 0;
			wmb();
			priv->hw->desc->set_tx_owner(desc);
			wmb();
		}
#if 0
		test_desc = (struct gmac_desc*)desc;
		printk("nfrags %d/%d : %d\n", i , nfrags, entry);
		printk("T[%03d] status: %08X\n", entry, test_desc->status);
		printk("T[%03d] length: %08X\n", entry, test_desc->length);
		printk("T[%03d] buffer1: %08X\n", entry, test_desc->buffer1);
		printk("T[%03d] buffer2: %08X\n", entry, test_desc->buffer2);
#endif
	}
...
#if 0 /* [Vin] net_device_ops Only in >= KERNEL_VERSION(2,6,29) */
 ...
 */
static int stmmac_probe(struct net_device *ndev)
{
	int ret = 0;
	...
	/* dev->fix_feature = stmmac_fix_features; */
	/* dev->set_rx_mode = stmmac_set_rx_mode; */
	ndev->set_multicast_list = stmmac_set_rx_mode;
	ndev->tx_timeout = stmmac_tx_timeout;
	ndev->do_ioctl = stmmac_ioctl;
	ndev->set_config = stmmac_config;
#ifdef CONFIG_NET_POLL_CONTROLLER
	ndev->poll_controller = stmmac_poll_controller;
#endif
	ndev->set_mac_address = stmmac_set_mac_address;
	...
	/* Get the MAC address */
	priv->hw->mac->get_umac_addr((void __iomem *) ndev->base_addr,
				     ndev->dev_addr, 0);
...
static int stmmac_dvr_probe(struct platform_device *pdev)
{
	...
	priv->plat = plat_dat;
	priv->ioaddr = addr;
	priv->dev->base_addr = (unsigned long)addr; /* ndev->base_addr */
	...
	ndev->irq = platform_get_irq_byname(pdev, "dwmac_irq");
	if (ndev->irq == -ENXIO) {
