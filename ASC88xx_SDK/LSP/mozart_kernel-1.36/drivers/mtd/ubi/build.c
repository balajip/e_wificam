...
int ubi_attach_mtd_dev(struct mtd_info *mtd, int ubi_num, int vid_hdr_offset)
{
	...
	err = -ENOMEM;
	ubi->peb_buf1 = Mozart_vmalloc(ubi->peb_size);
	if (!ubi->peb_buf1)
		goto out_free;

	ubi->peb_buf2 = Mozart_vmalloc(ubi->peb_size);
	if (!ubi->peb_buf2)
		goto out_free;

#ifdef CONFIG_MTD_UBI_DEBUG_PARANOID
	mutex_init(&ubi->dbg_buf_mutex);
	ubi->dbg_peb_buf = Mozart_vmalloc(ubi->peb_size);
	if (!ubi->dbg_peb_buf)
		goto out_free;
#endif
...
out_detach:
	...
	Mozart_vfree(ubi->vtbl);
out_free:
	Mozart_vfree(ubi->peb_buf1);
	Mozart_vfree(ubi->peb_buf2);
#ifdef CONFIG_MTD_UBI_DEBUG_PARANOID
	Mozart_vfree(ubi->dbg_peb_buf);
#endif
	if (ref)
		put_device(&ubi->dev);
	else
