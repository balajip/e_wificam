...
retry:
	...
	if (err) {
		//printk(KERN_DEBUG "paranoid_check_all_ff fail!\n") ;		
		//printk(KERN_DEBUG "bad block!!!!!!!!!!!!!!!\n") ;		
		//return err > 0 ? -EINVAL : err;
		return -EIO;
	} else {
		//printk(KERN_DEBUG "paranoid_check_all_ff ok\n") ;	
		;
	}

	if (ubi_dbg_is_erase_failure() && !err) {//random?
