/*tk831x.c - Linux kernel modules for sensortek stk8311/stk8312/stk8313 accelerometer
 *
...
#ifdef CONFIG_SENSORS_STK8312_MODULE
...
#define DONOT_SUPPORT_POWERSAVING 
...
#define STK_ACC_CALI_FILE "/mnt/flash/stk_acc_cali.conf"
...
{	
	...
	for ( i= 0; i < length; i++ )
		rxData[i] = i2c_smbus_read_byte_data( this_client, command+i );
...
static int STK_i2c_Tx(char *txData, int length)
{
	...
	#ifdef SMBUS
	if (i2c_smbus_write_block_data(this_client, txData[0], length-1, txData+1) < 0)
	#else
...
static int STK831x_SetEnable(struct stk831x_data *stk, char en)
{
	...
	if(stk->first_enable)
	{
		...
		else
		{
			...
			printk(KERN_INFO "\n%d: set offset:%d,%d,%d\n", __LINE__, offset[0], offset[1], offset[2]);
...
static int stk831x_irq_setup(struct i2c_client *client, struct stk831x_data *stk_int)
{
	...
	irq = 30;//gpio_to_irq(STK_INT_PIN);
	if ( irq < 0 )
	{
	...
	stk_int->irq = irq;	
        ...
        fLib_SetIntTrig(client->irq, LEVEL, H_ACTIVE);

	printk(KERN_INFO "%s: irq # = %d\n", __func__, irq);
	if(irq < 0)
		printk(KERN_ERR "%s: irq number was not specified!\n", __func__);
	error = request_irq(client->irq, stk_mems_irq_handler, IRQF_SHARED|IRQF_TRIGGER_RISING , "stk-mems", stk_int);
	if (error < 0) 
	{
...
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev); struct stk831x_data *stk = i2c_get_clientdata(client);
	if(atomic_read(&stk->enabled))
	{
		STK831x_SetEnable(stk, 0);
		atomic_set(&stk->enabled, 1);			
...
#ifndef  DONOT_SUPPORT_POWERSAVING
