...
const char vaudio_id[] = "$Vaudio: "NVP1114A_AUDIO_DRIVER_VERSION" "NVP1114A_AUDIO_PLATFORM" "__DATE__" "NVP1114A_AUDIO_COMPANY" $";

/* kernel i2c interface 2009.09.29 */
static struct i2c_client *i2c;
static struct i2c_client *vpl_codec_client;

#define NVP1114A_1ST_ADDR 0x70
#define NVP1114A_2ND_ADDR 0x72
...
static int NVP1114A_audio_capture_8bits[2] = {0} ; // 1 : yes, 0 : no
static int NVP1114A_audio_playback_8bits = 0 ;
...
static unsigned int samplerates[] = {8000, 16000};
...
void vpl_audio_write_i2cdev_reg(struct i2c_client *i2cdev, int reg, int val)
{
	...
	//printk( "vpl_audio_write_i2cdev_reg reg(0x%x) = val(0x%x)\n", reg, val) ;
...
int vpl_audio_read_i2cdev_reg(struct i2c_client *i2cdev, int reg)
{
	...
	unsigned char tmp_reg[1] = {0};
	tmp_reg[0] = (unsigned char)reg ;

	mutex_lock(&AV_SYNC_LOCK) ;

	i2c_master_send(i2cdev, tmp_reg, 1) ;
...
static void snd_i2s_audio_init(struct snd_i2s *chip, int device)
{
	...
	DEBUG("snd_i2s_audio_init i2s addr = 0x%x\n", virt_dev_base) ;
...
{	
	...
	struct i2c_client *hwparams_i2c_client = &vpl_codec_client[device_num] ;
	...
	switch(format) {
		case SNDRV_PCM_FORMAT_S16_LE :	
			DEBUG("[NVP1114A]SNDRV_PCM_FORMAT_S16_LE\n") ;
			if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
				//format
				tmp = vpl_audio_read_i2cdev_reg(hwparams_i2c_client, 0xB0) ;
				tmp &= ~(0x3 << 6) ;
				tmp |= (0x0 << 6) ;
				vpl_audio_write_i2cdev_reg(hwparams_i2c_client, 0xB0, tmp) ;
				//bit width
				tmp = vpl_audio_read_i2cdev_reg(hwparams_i2c_client, 0xAF) ;
				tmp &= ~(0x1 << 1) ;
				tmp |= (0x0 << 1) ;
				vpl_audio_write_i2cdev_reg(hwparams_i2c_client, 0xAF, tmp) ;

				NVP1114A_audio_playback_8bits = 0 ;
			}
			else {
				//format
				tmp = vpl_audio_read_i2cdev_reg(hwparams_i2c_client, 0xA6) ;
				tmp &= ~(0x3 << 6) ;
				tmp |= (0x0 << 6) ;
				vpl_audio_write_i2cdev_reg(hwparams_i2c_client, 0xA6, tmp) ;
				//bit width
				tmp = vpl_audio_read_i2cdev_reg(hwparams_i2c_client, 0xA5) ;
				tmp &= ~(0x1 << 1) ;
				tmp |= (0x0 << 1) ;
				vpl_audio_write_i2cdev_reg(hwparams_i2c_client, 0xA5, tmp) ;
				//set bit width for DMA reference
				NVP1114A_audio_capture_8bits[device_num] = 0 ;
			}			
			break ;
			
		case SNDRV_PCM_FORMAT_MU_LAW :
			DEBUG("[NVP1114A]SNDRV_PCM_FORMAT_MU_LAW\n") ;
			if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
				//format
				tmp = vpl_audio_read_i2cdev_reg(hwparams_i2c_client, 0xB0) ;
				tmp &= ~(0x3 << 6) ;
				...
				NVP1114A_audio_playback_8bits = 1 ;
			}
			else {
				//format
				tmp = vpl_audio_read_i2cdev_reg(hwparams_i2c_client, 0xA6) ;
				...
				vpl_audio_write_i2cdev_reg(hwparams_i2c_client, 0xA5, tmp) ;
				//set bit width for DMA reference
				NVP1114A_audio_capture_8bits[device_num] = 1 ;
			}			
			break ;
			
		case SNDRV_PCM_FORMAT_A_LAW :
			DEBUG("[NVP1114A]SNDRV_PCM_FORMAT_A_LAW\n") ;
			if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
				//format
				tmp = vpl_audio_read_i2cdev_reg(hwparams_i2c_client, 0xB0) ;
				tmp &= ~(0x3 << 6) ;
	...
	channel_num = params_channels(hw_params) ;
	DEBUG("channel_num = %d\n", channel_num) ;
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		rt->channels = channel_num ;
	}
	else {
		rt->channels = channel_num ;	
		switch(channel_num) {
		...
		vpl_audio_codec_capture_ch_num[device_num] = channel_num ;
	}

	/*printk("read all audio regs of codec0x%x..\n", hwparams_i2c_client->addr) ;
	for(i = 0 ; i < NVP1114A_CACHEREGNUM ; i++) {
		printk("reg[0x%x] = 0x%02x\n", i, vpl_audio_read_i2cdev_reg(hwparams_i2c_client, i)) ;
	}*/
			     ...
			     snd_pcm_uframes_t count)
{
	...
	int byte_num_of_sample = (NVP1114A_audio_capture_8bits[device_num]) ? 1 : 2 ;
...
{
	struct i2c_client *capture_i2c_client ;
	...
	//get volume
	capture_i2c_client = &vpl_codec_client[device_num] ;
	switch(channel_num) {
		case 0 :
			ori_volume = vpl_audio_read_i2cdev_reg(capture_i2c_client, 0xA3) ;
		...
		case 1 :
			...
			break ;
		case 2 :
			ori_volume = vpl_audio_read_i2cdev_reg(capture_i2c_client, 0xA4) ;
			ori_volume = ori_volume >> 4 ;
			ori_volume &= 0xF ;
			break ;
		case 3 :	
...
static int vpl_audio_capture_volume_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *value)
{
	...
	switch(channel_num) {
		case 0 :
			ori_volume = vpl_audio_read_i2cdev_reg(capture_i2c_client, 0xA3) ;
			ori_volume &= ~(0xF0) ;
			new_volume = (new_volume & 0x0F) << 4 ;
			new_volume = ori_volume | new_volume ;
			vpl_audio_write_i2cdev_reg(capture_i2c_client, 0xA3, new_volume);
			break ;
		case 1 :
			ori_volume = vpl_audio_read_i2cdev_reg(capture_i2c_client, 0xA3) ;
			ori_volume &= ~(0xF) ;
			new_volume = new_volume & 0x0F ;
			new_volume = ori_volume | new_volume ;
			vpl_audio_write_i2cdev_reg(capture_i2c_client, 0xA3, new_volume);
			break ;
		case 2 :
			...
			new_volume = (new_volume & 0x0F) << 4 ;
			new_volume = ori_volume | new_volume ;
			vpl_audio_write_i2cdev_reg(capture_i2c_client, 0xA4, new_volume);
			break ;
		case 3 :	
			ori_volume = vpl_audio_read_i2cdev_reg(capture_i2c_client, 0xA4) ;
			ori_volume &= ~(0xF) ;
...
static int vpl_audio_playback_volume_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *value)
{
	...
	volume = vpl_audio_read_i2cdev_reg(playback_client, 0xB3) & 0XF ;
	value->value.integer.value[0] = volume ; 
...
static int vpl_audio_playback_volume_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *value)
{
	...
	//set volume
	reg_val = new_volume & 0xF ;
	reg_val = (reg_val << 4) | reg_val ;
	vpl_audio_write_i2cdev_reg(playback_client, 0xB3, reg_val);
...
static int nvp1114a_codec_probe(struct i2c_adapter *adap, int addr, int kind)
{
	int ret;

	DEBUG("nvp1114a_codec_probe(%x)\n", addr) ;
	...
	if(ret < 0) {
		printk("[ERR]NVP1114A failed to attach at addr 0x%x\n", addr);
		i2c_counter++ ;
		goto err;
	}
...
#if 1		
		for(i = 0 ; i < (sizeof(tw2866_audio_regs_index) / sizeof(unsigned char)) ; i++) {
...
#endif		
		DEBUG("read all audio regs of codec0x%x..\n", vpl_codec_client[i2c_counter].addr) ;
		for(i = 0 ; i < NVP1114A_CACHEREGNUM ; i++) {
			DEBUG("reg[0x%x] = 0x%02x\n", i, vpl_audio_read_i2cdev_reg(&vpl_codec_client[i2c_counter], i)) ;
		}
	}	
...
static int nvp1114a_i2c_detach(struct i2c_client *client)
{
...
static int nvp1114a_i2c_attach(struct i2c_adapter *adap)
{
	return i2c_probe(adap, &addr_data, nvp1114a_codec_probe);
}

#define I2C_DRIVERID_NVP1114A 0xfefe /* need a proper id */
/* corgi i2c codec control layer */
static struct i2c_driver nvp1114a_i2c_driver = {
	.driver = {
		.name = "NVP1114A I2C Codec",
		.owner = THIS_MODULE,
	},
	.id =             I2C_DRIVERID_NVP1114A,
	.attach_adapter = nvp1114a_i2c_attach,
	.detach_client =  nvp1114a_i2c_detach,
	.command =        NULL,
};
...
static int snd_i2s_probe(struct platform_device *devptr)
{
	...
	ret = i2c_add_driver(&nvp1114a_i2c_driver);
	if (ret != 0) {
	...
	sprintf(card->shortname, "VPL Audio NVP1114A Driver");
	sprintf(card->longname, "VPL Audio Codec Driver, NVP1114A.");
...
static int __devexit snd_i2s_remove(struct platform_device *devptr)
{
	...
	/* kernel i2c interface 2009.09.29 */
	i2c_del_driver(&nvp1114a_i2c_driver);
	return 0;
}
...
MODULE_DESCRIPTION("______ Inc. NVP1114A Audio Codec Driver");
MODULE_LICENSE("GPL");
